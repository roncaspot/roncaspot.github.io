...
The title, description and Open Graph tags come from profile.json and are written into the
static HTML by the `page-meta` plugin in vite.config.js; do not set them at runtime.
public/og-image.jpg is generated by `npm run share-image`, which `npm run deploy` also runs.
